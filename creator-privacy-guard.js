...
import { getNullverseAccessContext, showGuestActionPrompt } from "./nullverse-guest.js?v=20260821";

let finalState = { denied: false, access: null, route: null };
let guestViewer = false;

try {
    const accessContext = await getNullverseAccessContext();
    guestViewer = Boolean(accessContext?.isGuest);
...
async function guardProtectedRoute(route) {
    if (route.type === "creator_gallery" && !route.username) {
        if (guestViewer) return { denied: false, access: null, route };
    ...
    if (error) {
        ...
        if (guestViewer) {
            const guestAccess = await enforceGuestCreatorPrivacy(route, null);
            if (guestAccess?.can_access === false) {
                try {
                    await renderLockedCreatorContent(guestAccess);
                } catch (renderError) {
                    console.error("Could not render the private creator lock screen:", renderError);
                    renderEmergencyPrivateLock(guestAccess);
                }
                return { denied: true, access: guestAccess, route, migrationMissing: true };
            }
        }
        // Existing authenticated behavior remains fail-open when the privacy migration
        // is absent; guest preview still independently enforces private-profile locks.
        return { denied: false, access: null, route, migrationMissing: true };
    }

    let access = Array.isArray(data) ? data[0] : data;

    if (guestViewer) {
        access = await enforceGuestCreatorPrivacy(route, access);
    }
...
async function enforceGuestCreatorPrivacy(route, access) {
    let ownerId = access?.owner_id || null;
    let username = access?.username || route?.username || "";

    if (!ownerId && route?.type === "creator_gallery" && username) {
        ...
        ownerId = data.id;
        access = { ...(access || {}), ...data, owner_id: data.id };
    }

    if (!ownerId && ["world", "literature", "comic"].includes(route?.type) && route?.contentId) {
        const { data } = await supabase
            .from("worlds")
            .select("owner_id")
            .eq("id", route.contentId)
            .maybeSingle();
        ownerId = data?.owner_id || null;
    }

    if (!ownerId && route?.type === "gallery_item" && route?.contentId) {
        const { data } = await supabase
    ...
    if (!owner || String(owner.profile_visibility || "public").toLowerCase() !== "private") {
        return access;
    }
    ...
    return {
        ...
        owner_id: owner.id,
        can_access: false,
        access_reason: "guest_private_profile"
    };
}
...
async function renderLockedCreatorContent(access) {
    ...
    overlay.innerHTML = `
        <div class="nv-private-content-card">
            ...
            : guestViewer
                ? `<button id="nv-private-guest-login" class="primary" type="button">Log In to Request</button>`
                : `<a class="primary" href="login.html">Beta Login to Request</a>`}
            </div>
        </div>`;
    document.body.appendChild(overlay);

    document.getElementById("nv-private-guest-login")?.addEventListener("click", () => {
        showGuestActionPrompt("request access to this creator");
    });
