...
export function isGuestBlockedAdultContent(record = {}) {
    const worldRating = String(record.content_rating || record.rating || "").trim().toLowerCase().replace(/\s+/g, "_");
    const galleryRating = String(record.age_rating || "").trim().toLowerCase().replace(/\s+/g, "_");
    const explicitFlag = record.is_nsfw === true || record.nsfw === true || record.is_adult === true || record.adult === true;

    // Worlds/Literature/Comics use content_rating=adult for 18+.
    // Gallery currently uses age_rating=mature as its 18+ value.
    return explicitFlag || ["adult", "18+", "18_plus", "explicit"].includes(worldRating) || ["adult", "18+", "18_plus", "mature"].includes(galleryRating);
}

export function filterGuestSafeContent(rows = [], isGuest = true) {
    if (!isGuest) return Array.isArray(rows) ? rows : [];
    return (Array.isArray(rows) ? rows : []).filter(row => !isGuestBlockedAdultContent(row));
}
